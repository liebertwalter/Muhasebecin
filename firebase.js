...
function addLogoutButton() {
  if (document.getElementById("logoutBtnTop")) return;
...
function updateAvatar(profile, user) {
  const avatar = document.querySelector(".avatar");
  if (!avatar) return;

  const source = profile?.username || user?.email || "M";
  avatar.innerText = source.charAt(0).toUpperCase();
}
...
async function syncUserData(user) {
  ...
  if (cloudData.length > 0) {
    ...
    await window.firebaseCloud.saveCloudData(user.uid, localData);
  } else {
    localStorage.setItem("muhasebecin_db", JSON.stringify([]));
  }
}
...
window.firebaseReady = new Promise((resolve) => {
  onAuthStateChanged(auth, async (user) => {
    ...
    if (user) {
      ...
      const profile = await window.firebaseCloud.loadProfile(user.uid);
      updateAvatar(profile, user);
      addLogoutButton();
